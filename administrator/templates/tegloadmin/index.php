...
{
	$logo = $this->baseurl . "/templates/" . $this->template . "/images/logo_weiss_mit_schatten.gif";
}
...
<body class="admin <?php echo $option . " view-" . $view . " layout-" . $layout . " task-" . $task . " itemid-" . $itemid . " ";?>" <?php if ($stickyToolbar): ?>data-spy="scroll" data-target=".subhead" data-offset="87"<?php endif;?>>
	...
	<script>
		(function($){
			...
			var $win = $(window)
			  , $nav = $('.subhead')
			  , navTop = $('.subhead').length && $('.subhead').offset().top - <?php if ($displayHeader || !$statusFixed): ?>80<?php else:?>20<?php endif;?>
			  , isFixed = 0
